...
//CAMBIA LA POSICION DE DOS BLOQUES
 ...
 //PARA ACTUALIZAR LA MATRIZ SUBIENDO 0 ARRIBA
  ...
  //PARA ACTUALIZAR MATRIZ DESPLAZANDO LAS COLUMNAS DE 0 A LA DERECHA
    ...
    comprobar(NFil,Ncol,lista2,op2)}
  }
  ...
  //JUGAR A LA PARTIDA
  def jugarPartida(creados:Int,vida:Int,pts:Int,FilasTotales:Int,ColTotales:Int,X:Int,Y:Int,tableroJuego:List[Int],dif:String,listaAux:List[Int]){
             ...
             print("\n PUNTOS ACUMULADOS: "+pts+"\n")
              if (terminar(tableroJuego)){
                ...
                if (buscarPosicion(0,(X*ColTotales+Y),tableroJuego)==8){
                    ...
                    if(X*ColTotales+Y<FilasTotales*ColTotales-1){
                  if(listaAux.length<bombas.length){
                    
                    if((X==FilasTotales-1) && (Y<(ColTotales-1))){
                      jugarPartida(creados,vida,pts,FilasTotales,ColTotales,0,Y+1,tableroJuego,dif,bombas)
                    }
                    else{
                      jugarPartida(creados,vida,pts,FilasTotales,ColTotales,X+1,Y,tableroJuego,dif,bombas)
                    }
                  
                  }else{
                    
                    if((X==FilasTotales-1) && (Y<(ColTotales-1))){
                      jugarPartida(creados,vida,pts,FilasTotales,ColTotales,0,Y+1,tableroJuego,dif,listaAux)
                    }
                    else{
                      jugarPartida(creados,vida,pts,FilasTotales,ColTotales,X+1,Y,tableroJuego,dif,listaAux)
                    }
                  }
                  }else{
                   
                
                   val puntos=puntuacion(listaAux)
                   val jrestantes= vidas(listaAux)
                   print("\n MATRIZ ACTUALIZADA\n")
                   val borrados= borrarVecinos(0,tableroJuego,listaAux.sortWith(_<_))
                   ...
                   print("\nVIDAS RESTANTES: \n")
                     print(vida)
                     print("\nINTRODUCE CUALQUIER NUMERO PARA CONTINUAR CON LA JUGADAS: ")
                 val ENTER=Integer.parseInt(String.valueOf(Console.in.readLine()))
                   Imprimir(ColTotales,repeticionC)
                   if (puntos==(-1)){jugarPartida(creados,vida-jrestantes,0,FilasTotales,ColTotales,0,0,repeticionC,dif,List())}
                   else{jugarPartida(creados,vida-jrestantes,pts+puntos,FilasTotales,ColTotales,0,0,repeticionC,dif,List())}
                  ...
                  if(X*ColTotales+Y<FilasTotales*ColTotales-1){
                  if(listaAux.length<vecinos.length){
                    
                    if((X==FilasTotales-1) && (Y<(ColTotales-1))){
                      jugarPartida(creados,vida,pts,FilasTotales,ColTotales,0,Y+1,tableroJuego,dif,vecinos)
                    }
                    else{
                      jugarPartida(creados,vida,pts,FilasTotales,ColTotales,X+1,Y,tableroJuego,dif,vecinos)
                    }
  ...
  case "BAJA" => val tableroInicial =  insertarNBombas(99,2,crearLista(3,99))
                 ...
                 jugarPartida(creados,8,puntos,9,11,0,0,tableroInicial,"BAJA",List())
  ...
  case "MEDIA" => val tableroInicial = insertarNBombas(192,3,crearLista(5,192))
                 ...
                 Imprimir(16,tableroInicial)
                    jugarPartida(creados,10,puntos,12,16,0,0,tableroInicial,"MEDIA",List())
                 
  case "ALTA" => val tableroInicial = insertarNBombas(375,5,crearLista(7,375))
  
                  print("\n *******NUEVO NIVEL******* \n")
                 Imprimir(15,tableroInicial)
                 jugarPartida(creados,15,puntos,25,15,0,0,tableroInicial,"ALTA",List())
  }
